...
def createWorkDir(id, path):
    try:
        workDir = Path(path)/str(id)
...
def run_mofem(exe, params, path):
    ...
    part_file = params.mesh_file + f"_{params.nproc}p.h5m"
...
if __name__=="__main__":
    # set the base path
    basePath = Path("/tmp/simulations")
    
    while True:
        users = [item for item in basePath.iterdir() if item.is_dir()]
        for user in users:
            database = db.getDb(basePath/f"{user}/database.json")
            for data in database.getAll():
                if data["status"] == "started":
                    if createWorkDir(data["id"], basePath):
                        data["status"] == "Created workDir"
                        workDir = Path(basePath)/str(data["id"])
                    else:
                        data["status"] == "Failed"
                if data["status"] == "Created workDir":
                    if generate_config(data["params"], workDir):
                        data["status"] == "Generated config"
                    else:
                        data["status"] == "Failed"
                if data["status"] == "Generated config":
                    if generate_mesh_box(data["params"], workDir):
                        data["status"] == "Generated mesh"
                    else:
                        data["status"] == "Failed"
